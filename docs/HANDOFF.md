...
v1.13.1。プロンプト交通整理と素材交通整理を1アプリに統合した最小構成に、
...
- inJustDecodeBounds の decodeStream は常に null を返す。`openInputStream(...)?.use { decodeStream(...) } ?: error(...)`
  と書くと必ず失敗する。ストリーム側で elvis を取ること（v1.13.1で修正した実際のバグ）
